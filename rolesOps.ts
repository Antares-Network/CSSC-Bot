...
export async function removePrevRole(
  member: GuildMember,
  listID: number
): Promise<void> {
  ...
  for (const role of Object.values(roleDictionary()[listID])) {
    if (member.roles.cache.has(role)) {
      ...
      console.log(
        chalk.green(
          `Removed role ${chalk.green(role)} from ${chalk.yellow(
            member.user.tag
          )}`
        )
      );
...
export async function addNewRole(
  ...
  console.log(
    chalk.green(
      `Added role ${chalk.green(role)} to ${chalk.yellow(member.user.tag)}`
    )
  );
}
...
export function checkForRoles(guild: Guild, listID: number): boolean {
  ...
  roleNames.forEach((roleName) => {
    ...
    if (!stateName && !stateId) {
      console.log(
        chalk.red.bold(
          `Role ${roleName} does not exist in ${guild.name}, Please run the /createRoles command in that server.`
        )
      );
  ...
  if (collection.includes(false)) {
    ...
    console.log(
      chalk.yellow.bold(`All roles in list ${listID} exist in ${guild.name}`)
    );
    return true;
  }
}
