...
async function getCourseInfo(srcdb: string) {
	...
	for (const course of courses) {
		...
		let entry: course = {
			code: course.code,
			title: course.title,
			description: description.replace(/<\/?[^>]+(>|$)/g, "").replace(".", ". "), // cleans all html tags and adds spaces after periods
		};
		info.push(entry);
	}
	return await info;
	...
	options: [
		{
			...
			required: true,
			choices: [ //TODO: Make this dynamic by querying the catalog api for all available semesters when the bot starts and then updating this list when the bot is restarted
				{
					name: "Spring 2023",
					value: "2232",
				},
	...
	callback: async ({ interaction }) => {
		...
		for (const entry of info) {
			...
			if (currentCourse) {
				...
				currentCourse.INFO = entry.description.replace(/<\/?[^>]+(>|$)/g, "").replace(".", ". "); // cleans all html tags and adds spaces after periods
